...
export function PageKpiGrid({
  ...
  return (
    <div className={cn("grid gap-3", GRID_COLS[cols], className)}>
      {items.map((s) => {
        const card = (
          <Card key={s.label} className={cn(PAGE_KPI_CARD_CLASS, s.tint, s.href && "cursor-pointer hover:border-primary/25 transition-colors")}>
            <CardContent className={cn("p-3 flex items-center gap-3 min-h-[4rem]")}>
        ...
        if (s.href) {
          return (
            <Link key={s.label} href={s.href} className="block focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary rounded-xl">
              {card}
            </Link>
          );
        }
