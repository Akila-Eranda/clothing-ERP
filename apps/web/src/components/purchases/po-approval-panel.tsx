  ...
  if (instance.status === "REJECTED") {
    return (
      <div className="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800">
        This purchase order was rejected during approval. Edit and submit again from Draft status.
      </div>
    );
  }

  return (
    <div className="rounded-xl border border-purple-200 bg-purple-50/80 p-4 space-y-4">
      <div className="flex items-start gap-3">
        <div className="p-2 rounded-lg bg-purple-100 shrink-0">
          <GitBranch className="h-5 w-5 text-purple-700" />
        </div>
        <div className="flex-1 min-w-0 space-y-1">
          <p className="font-semibold text-sm text-purple-900">Approval in progress</p>
          {message && <p className="text-xs text-purple-800">{message}</p>}
          <p className="text-xs text-purple-700/80">
            GRN and receiving stay blocked until both steps are approved.
          </p>
        </div>
          ...
          {steps.map((step, i) => {
            ...
            return (
              <React.Fragment key={step.stepOrder}>
                {i > 0 && <div className="hidden sm:block w-6 h-px bg-purple-200 self-center shrink-0" />}
                <div
                  className={`flex-1 rounded-lg border px-3 py-2 text-xs ${
                    state === "done"
                      ? "border-emerald-200 bg-emerald-50 text-emerald-800"
                      : state === "active"
                        ? "border-purple-300 bg-white text-purple-900 ring-1 ring-purple-200"
                ...
                >
                  <div className="flex items-center gap-1.5 font-medium">
                    {state === "done" ? (
                      ...
                      <Clock className="h-3.5 w-3.5 text-purple-600 shrink-0 animate-pulse" />
                    ) : (
                      <span className="h-3.5 w-3.5 rounded-full border border-purple-300 shrink-0" />
                    )}
                    {step.name}
                  </div>
      ...
      {canAct && pendingTaskId && onApprove && onReject && (
        <div className="flex flex-wrap items-center gap-2 pt-1 border-t border-purple-200/80">
          <span className="text-xs text-purple-800 mr-auto">
