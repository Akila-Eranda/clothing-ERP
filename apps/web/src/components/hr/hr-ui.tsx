...
export const HR_STAT_PRESETS = {
  ...
  emerald: {
    ...
    tint: "border-emerald-200/80 bg-gradient-to-br from-emerald-50 to-white dark:border-emerald-500/20 dark:from-emerald-500/10 dark:to-transparent",
  },
  amber: {
  ...
  red: {
    ...
    bg: "bg-red-500/15",
    tint: "border-red-200/80 bg-gradient-to-br from-red-50 to-white dark:border-red-500/20 dark:from-red-500/10 dark:to-transparent",
  },
  teal: {
    color: "text-teal-600 dark:text-teal-400",
    bg: "bg-teal-500/15",
    tint: "border-teal-200/80 bg-gradient-to-br from-teal-50 to-white dark:border-teal-500/20 dark:from-teal-500/10 dark:to-transparent",
  },
} as const;

export type HrStatPreset = keyof typeof HR_STAT_PRESETS;
...
export function hrStat(
  ...
  const p = HR_STAT_PRESETS[preset];
...
export function HrStatCards({
  ...
  const gridClass =
    cols === 2 ? "grid-cols-2" : cols === 3 ? "grid-cols-2 xl:grid-cols-3" : "grid-cols-2 xl:grid-cols-4";
  const skeletonCount = Math.min(stats.length || cols, cols);

  if (loading) {
    return (
      <div className={cn("grid gap-3", gridClass)}>
        {Array.from({ length: skeletonCount }).map((_, i) => (
          <Card key={i} className={HR_CARD_CLASS}>
  ...
  return (
    <div className={cn("grid gap-3", gridClass)}>
      {stats.map((s) => (
        <Card key={s.label} className={cn(HR_CARD_CLASS, s.tint)}>
          <CardContent className="h-[68px] p-4 flex items-center gap-3">
...
export function HrPageShell({ children, className }: { children: React.ReactNode; className?: string }) {
  return <div className={cn("page-shell space-y-5", className)}>{children}</div>;
}

export function HrPanel({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <div className={cn("rounded-[18px] border bg-card p-4 shadow-[0_2px_10px_rgba(15,23,42,0.04)]", className)}>
      {children}
    </div>
  );
}
