...
import {
  Building2, Users, TrendingUp, Activity, RefreshCw,
...
import { fetchTenants, fetchPlatformStats, fetchHealth, type TenantRow, type PlatformStats, type HealthData } from '@/lib/admin-api'
...
export default function AdminDashboardPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError('')
    try {
      const [s, tRes, h] = await Promise.all([
        fetchPlatformStats(),
        fetchTenants({ limit: '10', sort: 'createdAt', order: 'desc' }),
  ...
  const statCards = stats ? [
    ...
    { label: 'Active Tenants',   value: stats.activeTenants,   icon: CheckCircle, color: 'text-green-600',  bg: 'bg-green-50'   },
    { label: 'Suspended',        value: stats.suspendedTenants,icon: XCircle,    color: 'text-amber-600',   bg: 'bg-amber-50'   },
    { label: 'Total Users',      value: stats.totalUsers,      icon: Users,      color: 'text-violet-600',  bg: 'bg-violet-50'  },
    { label: 'New This Month',   value: stats.newThisMonth,    icon: TrendingUp, color: 'text-cyan-600',    bg: 'bg-cyan-50'    },
    { label: 'System Health',    value: health?.status === 'ok' ? 'Healthy' : 'Check', icon: Activity, color: health?.status === 'ok' ? 'text-green-600' : 'text-red-600', bg: health?.status === 'ok' ? 'bg-green-50' : 'bg-red-50' },
  ] : []

  const planDonut = stats?.planBreakdown.filter(p => p.count > 0).map(p => ({
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5 lg:col-span-2">
          ...
          {loading ? <Skeleton h="h-48" /> : (
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={stats?.planBreakdown ?? []} barSize={40}>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100">
                ...
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-gray-500 uppercase tracking-wide">Subdomain</th>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-gray-500 uppercase tracking-wide">Plan</th>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-gray-500 uppercase tracking-wide">Status</th>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-gray-500 uppercase tracking-wide">Users</th>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-gray-500 uppercase tracking-wide">Joined</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50">
              {loading ? (
                ...
                <tr key={t.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-xs text-gray-600">{t._count?.users ?? '—'}</td>
