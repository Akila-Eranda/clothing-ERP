...
export default function JobCardsPage() {
  ...
  const STATS = [
    { label: "Total Jobs",     value: jobs.length,                        icon: ClipboardList, color: "text-blue-500",    bg: "bg-blue-500/10" },
    { label: "Open",           value: openCount,                          icon: Clock,         color: "text-amber-500",   bg: "bg-amber-500/10" },
    { label: "In Progress",    value: activeCount,                        icon: Wrench,        color: "text-violet-500",  bg: "bg-violet-500/10" },
    { label: "Workshop Value", value: `LKR ${formatNumber(totalValue)}`,  icon: Banknote,      color: "text-emerald-500", bg: "bg-emerald-500/10" },
  ];
      ...
      <div className="p-4 md:p-5 space-y-4 max-w-[1600px] mx-auto">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex items-center gap-2 flex-wrap shrink-0">
            <Button variant="outline" onClick={fetchJobs} className="h-10 rounded-[12px] gap-1.5 text-sm">
            ...
            <Button className="h-10 rounded-[12px] gap-1.5 text-sm" onClick={() => { resetForm(); setCreateOpen(true); }}>
