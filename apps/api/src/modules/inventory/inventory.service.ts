...
export class InventoryService {
  ...
  async adjustStock(
    ...
    const execute = async (client: Prisma.TransactionClient) => {
      ...
      const computed = computeStockDelta({
        movementType: dto.movementType,
        quantity: dto.quantity,
        currentQty,
        currentDamaged,
        currentReturned,
        allowNegativeSale: allowNegSale,
      });
      try {
        assertSufficientStock(computed, {
          movementType: dto.movementType,
          quantity: dto.quantity,
          currentQty,
          currentDamaged,
      ...
      const { delta, newDamaged, newReturned, finalQty } = computed;
  ...
  /**
   * Inventory Engine — create/update opening inventory rows for new variants.
   * Prefer this over direct prisma.inventory writes from Products.
   */
  async seedOpeningStock(rows: OpeningStockSeed[]): Promise<{ created: number }> {
  ...
  async applyInventoryStockPrefs(
    tenantId: string,
    variantIds: string[],
    branchIds: string[],
    prefs: {
    ...
    if (!variantIds.length || !branchIds.length) return;
    const data: Record<string, number> = {};
    if (prefs.quantity != null) data.quantity = prefs.quantity;
    if (prefs.reorderPoint != null) data.reorderPoint = prefs.reorderPoint;
    if (prefs.minStockLevel != null) data.minStockLevel = prefs.minStockLevel;
    if (prefs.maxStockLevel != null) data.maxStockLevel = prefs.maxStockLevel;
