/** Notification Engine — in-app delivery, planned alerts, SMS outbox stub. */
...
export class NotificationsService {
  ...
  /**
   * Send a planned alert with dedupe. Mutates `existing` when sent.
   * Public API for scans and domain callers.
   */
  async notifyFromPlan(
    tenantId: string,
    alert: PlannedAlert,
    recipientIds: string[],
    existing: Set<string>,
  ...
  /** SMS outbox stub — persists channel=SMS row; no gateway yet. */
  ...
  async managerIds(tenantId: string, includeCashier = false) {
    const types = recipientRoleTypes(includeCashier);
    const users = await this.prisma.user.findMany({
      where: {
        tenantId,
        status: 'ACTIVE',
        roles: { some: { role: { type: { in: [...types] as never[] } } } },
      },
      select: { id: true },
    });
  ...
  async recentDedupeKeys(tenantId: string, sinceHours = 24): Promise<Set<string>> {
  ...
  async handleLowStock(payload: {
    ...
    await this.notifyFromPlan(payload.tenantId, alert, admins, existing);
    ...
    if (isReorderNeeded(payload.quantity, payload.reservedQty ?? 0, payload.reorderPoint)) {
      ...
      await this.notifyFromPlan(payload.tenantId, reorder, admins, existing);
    }
  }

  @OnEvent('pos.day.closed')
  async handleDayClosed(payload: {
    tenantId: string;
    closedBy: string;
    totalRevenue: number;
    totalSales: number;
  ...
  private async scanTenantAlerts(tenantId: string): Promise<number> {
    ...
    for (const inv of inventory) {
      if (!isLowStock(inv.quantity, { reorderPoint: inv.reorderPoint, minStockLevel: inv.minStockLevel })) {
        continue;
      }
      ...
      if (await this.notifyFromPlan(tenantId, low, admins, existing)) sent += 1;
      ...
      if (isReorderNeeded(inv.quantity, inv.reservedQty, inv.reorderPoint)) {
        ...
        if (await this.notifyFromPlan(tenantId, reorder, admins, existing)) sent += 1;
      }
    }
    ...
    for (const lot of lots) {
      ...
      if (await this.notifyFromPlan(tenantId, alert, admins, existing)) sent += 1;
    }
  ...
  private async scanDailySummary(tenantId: string): Promise<number> {
    ...
    return (await this.notifyFromPlan(tenantId, alert, admins, existing)) ? 1 : 0;
  }
}
